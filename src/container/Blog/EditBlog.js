...
const EditBlog = ({ onChange, onSend, defaultTag, replay, text }) => {
    const Location = useLocation();
    const values = Location.state;
    console.log(values)
  const [state, setState] = useState({
    desc: RichTextEditor.createValueFromString(values.desc,'html'),
    tags: values.tags|| defaultTag ? [defaultTag] : [],
    ...
    <MailBox>
      <div className="body" style={{ height: '100%' }}>
        ...
        <div className="left d-flex align-items-center" style={{ padding: '1rem', marginLeft: '2rem' }}>
          <div to="">
            <p style={{ color: '#000' }}>Blog Image</p>
            {
                state.doc_key !== '' ? (
                    <img
                        alt={state.title}
                        style={{ height: 280 }}
                        src={`https://business-easy-bucket.s3.us-east-2.amazonaws.com/${state.doc_key}`}
                        />
                ) : (
                    <Upload {...props}>
