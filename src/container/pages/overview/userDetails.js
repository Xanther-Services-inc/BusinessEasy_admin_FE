...
const UserDetails = ({ match }) => {
  const userLogin = useSelector((state) => state.auth)
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            let {data} = await axios.get(`${process.env.REACT_APP_API}/api/v1/user/user-details-orders?email=${match.params.email}`)
  ...
  <Card.Body>
    ...
    <Card.Subtitle className="mb-2 text-muted">📭 {order.email}</Card.Subtitle>
    <Card.Subtitle className="mb-2 text-muted">📱 {order.phone}</Card.Subtitle>
    <Card.Subtitle className="mb-2 text-muted">🗃 {order.pan}</Card.Subtitle>
    <Card.Subtitle className="mb-2 text-muted">🏳️‍🌈 {order.country + ', ' + order.state + ', ' + order.city}</Card.Subtitle>
    <Card.Subtitle className="mb-2 text-muted">🛍 {order.product_id}</Card.Subtitle>
    <Card.Text>💰 {order.price + ' [' + order.payment + ']'}
    </Card.Text>
    {order.doc_key.map(doc => (
      <>
