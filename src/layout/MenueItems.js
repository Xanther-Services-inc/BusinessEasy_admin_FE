...
const MenuItems = ({ darkMode, toggleCollapsed, topMenu, events }) => {
  ...
  return (
    ...
    >
      ...
      {!topMenu && <p className="sidebar-nav-title">{(isLoggedInJsonObject).type === 'super_user' ? "Super User View" : (isLoggedInJsonObject).type === 'b_manager' ? "Business Manager View" : "Employee View"}</p>}
      {!topMenu && <p className="sidebar-nav-title">Applications</p>}
      {isLoggedInJsonObject && ((isLoggedInJsonObject).type === 'super_user' || (isLoggedInJsonObject).type === 'b_manager')  ? 
